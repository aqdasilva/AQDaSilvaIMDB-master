...
def test_enter_data():
    ...
    connection, db_cursor = dbase.open_db("testDatabase.sqlite")
    dbase.create_top250TV_table(db_cursor)
    dbase.put_top_250_in_database("top_show_data", test_data_entry, db_cursor)
    connection.commit()
...
def test_table_exists():
    ...
    connection, db_cursor = dbase.open_db("testDatabase.sqlite")
    dbase.create_most_pop_movie_table(db_cursor)
...
def test_put_most_popular():
    ...
    dbase.put_most_popular_in_database("most_popular_movies", test_pop_data_entry, db_cursor)
    connection.commit()
...
def test_foriegn_key():
    ...
    connection, db_cursor = dbase.open_db("testDatabase.sqlite")
    dbase.create_top250TV_table(db_cursor)
    dbase.create_tv_ratings_table(db_cursor)
