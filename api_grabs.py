...
def sqlite_convert_movies_csv_file():
    conn = sqlite3.connect("project1db.sqlite")
    conn.text_factory = str
    cur = conn.cursor()
    data = cur.execute("SELECT * FROM most_popular_shows ")

    with open('popularMovies.csv', 'wb') as f:
        writer = csv.writer(f)
        writer.writerow(['ttid', 'rank', 'rankchange', 'title'])
        writer.writerows(data)

def sqlite_convert_shows_to_csv_file():
    conn = sqlite3.connect("project1db.sqlite")
    conn.text_factory = str
    cur = conn.cursor()
    data = cur.execute("SELECT * FROM most_popular_shows ")

    with open('popularShows.csv', 'wb') as f:
        writer = csv.writer(f)
        writer.writerow(['ttid', 'rank', 'rankchange', 'title'])
        writer.writerows(data)
